...
def choose_move(data: dict) -> str:
    ...
    for item in my_body:
      if ("left" in possible_moves) & (item["x"] == my_head["x"] - 1):
        possible_moves.remove("left")

      if ("right" in possible_moves) & (item["x"] == my_head["x"] + 1):
        possible_moves.remove("right")

      if ("up" in possible_moves) & (item["y"] == my_head["y"] + 1):
        possible_moves.remove("up")

      if ("down" in possible_moves) & (item["y"] == my_head["y"] - 1):
        possible_moves.remove("down")
